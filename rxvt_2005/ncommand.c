...
#endif
int vlbi_on = 1; /* this is 1 if accept input from FS9 and VME */
...
static unsigned char *lookup_key(XEvent *ev,int *pcount)
{
 ...
 /*     strcpy(kbuf,"\033[11~");
      count = 5;  */
/* eff eff eff -------F1 (F1/F6 on usual PC)------------- */
      if (vlbi_on == 1){ vlbi_on=0;
        ...
        count=0;  /*this  adds nothing to buffer*/
/* eff eff eff ends------------------------- */
      break;
    case XK_F2 :
      strcpy(kbuf,"\033[12~");
...
/* look for something coming in from VME or FS9 field systems */
        if (vlbi_pending == 1)  /* is last line still not sent ? */
        {
           if(vlbi_on == 1)
           {
           send_effberg(sockbuf,nread,1); /* pending line out to vax*/
        ...
        if(FD_ISSET(socket_fd,&in_fdset))
        {
           ...
           if(vlbi_on == 1)
                send_effberg(sockbuf,nread,0); /* out to vax*/
